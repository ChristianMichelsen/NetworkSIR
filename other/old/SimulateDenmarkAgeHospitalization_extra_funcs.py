...
do_only_which_connections = True
...
class DotDict2(dict):
    ...
    Class that allows a dict to indexed using dot-notation.
    Example:
    >>> dotdict = DotDict2({'first_name': 'Christian', 'last_name': 'Michelsen'})
    >>> dotdict.last_name
    'Michelsen'
    """
...
class Filename:
    ...
    def to_dict(self): # , 
        return DotDict2({key: val for key, val in self.d.items() if key != 'ID'})
...
if False:
    ...
    simulation._Filename
    simulation.cfg
    simulation.ID
    simulation._memory_file
...
class Simulation:
    ...
    def _prepare_memory_file(self):
        ...
        if self.ID == 0:
            make_sure_folder_exist(self._memory_file) # make sure parent folder exists
            delete_file(self._memory_file)
    ...
    def _initialize_network(self):
        ...
        which_connections = initialize_nested_lists(cfg.N_tot, dtype=np.uint32) # initialize_list_set
        N_connections = np.zeros(cfg.N_tot, dtype=np.uint16)
        individual_rates = initialize_nested_lists(cfg.N_tot, dtype=np.float32)
        ...
        ages, ages_total_counts, ages_in_state, PT_ages, PC_ages, PP_ages = initialize_ages(cfg.N_tot, cfg.N_ages, cfg.connection_weight)
        ...
        connect_nodes(cfg.mu, cfg.epsilon_rho, cfg.rho, cfg.algo, PP_ages, N_connections, individual_rates, which_connections, which_connections_reference, self.coordinates, infection_weight, N_connections_reference, rho_scale, N_ages, age_matrix, ages_in_state, verbose)
